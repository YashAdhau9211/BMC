...
| NFR-5 Data locality | All corpus data + derived artifacts remain on controlled infrastructure; **no corpus content sent to external APIs unless the corpus is explicitly cleared for it** (config flag `external_models_allowed`, default false) |
...
| NFR-12 Licensing | All models/components under licenses permitting internal commercial use; license inventory maintained |
...
```
  ...
  Web UI (React) ──────────────►│ API Gateway │
        ...
        │ (vLLM: LLM/ │ │BM25+filt.│ │dense +  │ │ KG +    │
        │ SLM/VLM/    │ └─────────┘ │multivec  │ │ GDS     │
        │ rerank/embed│             │(ColQwen) │ └─────────┘
        └─────────────┘             └─────────┘
                ▲
        ┌───────┴────────┐        ┌──────────────────┐
...
Single `pipeline.yaml` per environment: model names+versions, prompts (referenced by file+hash), chunking params, fusion weights, k values, budgets, feature flags (`visual_index`, `graph`, `verification`, `external_models_allowed`). Every trace records the config hash it ran under. **Acceptance: two runs with the same config hash on the same index are reproducible (modulo sampling temperature, which is 0 for graders/routers).**
...
| Item | POC provision |
|---|---|
| Compute | 1 GPU server (2× A100/H100-class or 4× L40S): vLLM fleet (7B–32B LLM, VLM, SLMs, reranker, embedder) + 1 GPU slice for ColQwen indexing bursts; 1 CPU server (32–64 vCPU, 256 GB) for OpenSearch+Qdrant+Neo4j+services; object store (MinIO/S3) for raw + renders + frames |
...
## 10. Cost & Budget Guardrails (planning priors — refine after Phase 1)

| Item | Budget guardrail |
|---|---|
| Fast-mode query | ≤ $0.02 equiv. compute (tokens + GPU-s) |
| Investigate query | ≤ $0.50 equiv.; hard budget caps enforce (FR-AGT-3) |
| Phase-3 visual indexing | ≤ 1 GPU-week for POC slice; pooling/quantization mandatory if exceeded |
| Phase-4 extraction | SLM extractor only for bulk; frontier-class models only for prompt development on samples |
| Monthly infra | Within agreed envelope (set at kickoff); weekly cost report from traces |
...
2. **External model policy** — is any corpus subset cleared for hosted frontier APIs (`external_models_allowed`)? Affects synthesis quality ceiling and §10.
